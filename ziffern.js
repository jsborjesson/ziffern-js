...
Opal.queue(function(Opal) {/* Generated by Opal 1.8.2 */
  var $klass = Opal.klass, $const_set = Opal.const_set, $send = Opal.send, $def = Opal.def, $truthy = Opal.truthy, $eqeqeq = Opal.eqeqeq, $range = Opal.range, $to_ary = Opal.to_ary, $rb_plus = Opal.rb_plus, $rb_gt = Opal.rb_gt, $eqeq = Opal.eqeq, $nesting = [], nil = Opal.nil;

  Opal.add_stubs('freeze,flat_map,new,validate_number!,join,compact,convert_sign,convert_integer,convert_decimals,private,Float,raise,negative?,to_f,sub,convert,abs,to_i,===,[],twenty_to_99,quantify_by_factor,convert_large_number,divmod,zero?,+,convert_millions,large_number_groups,large_number_names,count,>,size,fail,map,reject,zip,quantify_large_number,feminine_convert,pluralize_large_number,==,<<,reverse,take,get_decimals_as_string,empty?,convert_digits,to_s,chars');
  return (function($base, $super, $parent_nesting) {
    var self = $klass($base, $super, 'Ziffern');

    var $nesting = [self].concat($parent_nesting), $$ = Opal.$r($nesting);

    
    $const_set($nesting[0], 'AND', "und".$freeze());
    $const_set($nesting[0], 'MINUS', "minus".$freeze());
    $const_set($nesting[0], 'POINT', "Komma".$freeze());
    $const_set($nesting[0], 'HUNDRED', "hundert".$freeze());
    $const_set($nesting[0], 'THOUSAND', "tausend".$freeze());
    $const_set($nesting[0], 'FIRST_TWENTY', ["null", "ein", "zwei", "drei", "vier", "fünf", "sechs", "sieben", "acht", "neun", "zehn", "elf", "zwölf", "dreizehn", "vierzehn", "fünfzehn", "sechzehn", "siebzehn", "achtzehn", "neunzehn"].$freeze());
    $const_set($nesting[0], 'TENS', ["WRONG_INDEX", "WRONG_INDEX", "zwanzig", "dreißig", "vierzig", "fünfzig", "sechzig", "siebzig", "achtzig", "neunzig"].$freeze());
    $const_set($nesting[0], 'LARGE_NUMBERS', $send(["M", "B", "Tr", "Quadr", "Quint", "Sext", "Sept", "Okt", "Non", "Dez", "Undez", "Dodez", "Tredez", "Quattuordez", "Quindez", "Sedez", "Septendez", "Dodevigint", "Undevigint", "Vigint"], 'flat_map', [], function $Ziffern$1(prefix){
      
      if (prefix == null) prefix = nil;
      return ["" + (prefix) + "illion", "" + (prefix) + "illiarde"];}).$freeze());
    $const_set($nesting[0], 'TooLargeNumberError', $$('Class').$new($$('ArgumentError')));
    $const_set($nesting[0], 'InvalidNumberError', $$('Class').$new($$('ArgumentError')));
    
    $def(self, '$to_german', function $$to_german(number) {
      var self = this;

      
      self['$validate_number!'](number);
      return [self.$convert_sign(number), self.$convert_integer(number), self.$convert_decimals(number)].$compact().$join(" ");
    });
    self.$private();
    
    $def(self, '$validate_number!', function $Ziffern_validate_number$excl$2(number) {
      var self = this;
      ...
      try {
        ...
        if (Opal.rescue($err, [$$('ArgumentError')])) {
          try {
            return self.$raise($$('InvalidNumberError'))
          } finally { Opal.pop_exception($err); }
        } else { throw $err; }
      }
    });
    
    $def(self, '$convert_sign', function $$convert_sign(number) {
      
      if ($truthy(number.$to_f()['$negative?']())) {
        return $$('MINUS')
      } else {
        return nil
      }
    ...
    $def(self, '$convert_integer', function $$convert_integer(number) {
      var self = this;

      return self.$convert(number.$to_i().$abs()).$sub(/(ein)$/, "eins")
    });
    
    $def(self, '$convert', function $$convert(number) {
      var self = this, $ret_or_1 = nil;

      if ($eqeqeq($range(0, 19, false), ($ret_or_1 = number))) {
        return $$('FIRST_TWENTY')['$[]'](number)
      } else if ($eqeqeq($range(20, 99, false), $ret_or_1)) {
        return self.$twenty_to_99(number)
      } else if ($eqeqeq($range(100, 999, false), $ret_or_1)) {
        return self.$quantify_by_factor(100, $$('HUNDRED'), number)
      } else if ($eqeqeq($range(1000, 999999, false), $ret_or_1)) {
        return self.$quantify_by_factor(1000, $$('THOUSAND'), number)
      } else {
        return self.$convert_large_number(number)
      }
    });
    
    $def(self, '$twenty_to_99', function $$twenty_to_99(number) {
      var $a, $b, self = this, ten = nil, remainder = nil;

      
      $b = number.$divmod(10), $a = $to_ary($b), (ten = ($a[0] == null ? nil : $a[0])), (remainder = ($a[1] == null ? nil : $a[1])), $b;
      if ($truthy(remainder['$zero?']())) {
        return $$('TENS')['$[]'](ten)
      } else {
        return [self.$convert(remainder), $$('AND'), $$('TENS')['$[]'](ten)].$join()
      };
    });
    
    $def(self, '$quantify_by_factor', function $$quantify_by_factor(factor, quantifier, number) {
      var $a, $b, self = this, amount = nil, remainder = nil, text = nil;

      
      $b = number.$divmod(factor), $a = $to_ary($b), (amount = ($a[0] == null ? nil : $a[0])), (remainder = ($a[1] == null ? nil : $a[1])), $b;
      text = $rb_plus(self.$convert(amount), quantifier);
      if (!$truthy(remainder['$zero?']())) {
        text = $rb_plus(text, self.$convert(remainder))
      };
      return text;
    });
    
    $def(self, '$convert_large_number', function $$convert_large_number(number) {
      var $a, $b, self = this, number_of_millions = nil, remainder = nil, text = nil;

      
      $b = number.$divmod(1000000), $a = $to_ary($b), (number_of_millions = ($a[0] == null ? nil : $a[0])), (remainder = ($a[1] == null ? nil : $a[1])), $b;
      text = self.$convert_millions(number_of_millions);
      if (!$truthy(remainder['$zero?']())) {
        text = "" + (text) + " " + (self.$convert_integer(remainder))
      };
      return text;
    });
    
    $def(self, '$convert_millions', function $$convert_millions(number_of_millions) {
      var self = this, large_number_groups = nil, large_number_names = nil;

      
      large_number_groups = self.$large_number_groups(number_of_millions);
      large_number_names = self.$large_number_names(large_number_groups.$count());
      if ($truthy($rb_gt(large_number_groups.$size(), $$('LARGE_NUMBERS').$size()))) {
        self.$fail($$('TooLargeNumberError'))
      };
      return $send($send(large_number_groups.$zip(large_number_names), 'reject', [], function $$3(amount_name){
        
        if (amount_name == null) amount_name = nil;
        return amount_name['$[]'](0)['$zero?']();}), 'map', [], function $$4(amount, name){var self = $$4.$$s == null ? this : $$4.$$s;

        
        if (amount == null) amount = nil;
        if (name == null) name = nil;
        return self.$quantify_large_number(amount, name);}, {$$s: self}).$join(" ");
    });
    
    $def(self, '$quantify_large_number', function $$quantify_large_number(amount, large_number_name) {
      var self = this;

      return [self.$feminine_convert(amount), self.$pluralize_large_number(amount, large_number_name)].$join(" ")
    });
    
    $def(self, '$feminine_convert', function $$feminine_convert(number) {
      var self = this;
    ...
    $def(self, '$pluralize_large_number', function $$pluralize_large_number(amount, large_number_name) {
      
      
      if ($eqeq(amount, 1)) {
        return large_number_name
      };
      return large_number_name.$sub(/(e?)$/, "en");
    });
    
    $def(self, '$large_number_groups', function $$large_number_groups(number_of_millions) {
      var $a, $b, result = nil, remainder = nil, large_number_group = nil;

      
      result = [];
      remainder = number_of_millions;
      while (!($truthy(remainder['$zero?']()))) {
      
        $b = remainder.$divmod(1000), $a = $to_ary($b), (remainder = ($a[0] == null ? nil : $a[0])), (large_number_group = ($a[1] == null ? nil : $a[1])), $b;
        result['$<<'](large_number_group);
      };
      return result.$reverse();
    });
    
    $def(self, '$large_number_names', function $$large_number_names(count) {
      
      return $$('LARGE_NUMBERS').$take(count).$reverse()
    });
    
    $def(self, '$convert_decimals', function $$convert_decimals(number) {
      var self = this, decimals = nil;

      
      decimals = self.$get_decimals_as_string(number);
      if ($truthy(decimals['$empty?']())) {
        return nil
      };
      return [$$('POINT'), self.$convert_digits(decimals)].$join(" ");
    });
    
    $def(self, '$get_decimals_as_string', function $$get_decimals_as_string(number) {
      
      return number.$to_s()['$[]'](/\.(\d+)/, 1).$to_s()
    });
    return $def(self, '$convert_digits', function $$convert_digits(number) {
      var self = this;

      return $send(number.$to_s().$chars(), 'map', [], function $$5(digit){var self = $$5.$$s == null ? this : $$5.$$s;

        
        if (digit == null) digit = nil;
        return self.$convert_integer(digit);}, {$$s: self}).$join(" ")
    });
  })($nesting[0], null, $nesting)
});
